...
SSHDriverStatus SSHDriver::write(const char *buffer, size_t bufferSize, size_t *bytesWritten, int timeout)
{
  ...
  *bytesWritten = 0;
  ...
  while ((bytesToRead > 0) && (tnow < mtimeout)){
    ...
    if (rc > 0){
      /* AS2 NA220131 fix echo read issue [MSP-215]
       checks and corrects indices if there are additional \r or \n echoed. 
       This fixes the most frequent parse issues for fast reads.
      */
      
      if (bytes==0){
        while (  (extraCrCount < rc + 1) && ( (buff[extraCrCount] == '\r') || (buff[extraCrCount] == '\n') )  ){
          extraCrCount += 1;
        }
        bytesToRead += extraCrCount;
      }
