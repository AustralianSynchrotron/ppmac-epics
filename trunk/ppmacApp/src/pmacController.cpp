...
void pmacController::createAsynParams(void) {
  ...
  createParam(PMAC_C_MotorFErrorString, asynParamFloat64, &PMAC_C_MotorFError_);
...
void pmacController::setupBrokerVariables(void) {
  ...
  /* [MSP-200] 
   can't add the feedrates now, as some Cs might not have yet created
   it will be done later in medium update
...
asynStatus pmacController::mediumUpdate(pmacCommandStore *sPtr) {
  ...
  for (int csNo = 1; csNo <= PMAC_MAX_CS - 1; csNo++) {
    ...
    if (pCSControllers_[csNo] != NULL) {
      ...
      //[MSP-200] add Cs for feedrate fetching 
      if ( not sPtr->checkForItem(command) ) {
        debugf(DEBUG_VARIABLE, functionName, "Adding [&%d%s] to medium update read cycle", csNo);
        ...
        if (feedRate == "") {
          debug(DEBUG_ERROR, functionName, "Problem: feed rate command %s returned empty", command);
          status = asynError;
        } else {
...
asynStatus pmacController::writeFloat64(asynUser *pasynUser, epicsFloat64 value) {
  ...
  /* ----- [MSP-166] detach motrec DHLM/DLLM from controller MaxPos/MinPos
...
asynStatus pmacController::writeInt32(asynUser *pasynUser, epicsInt32 value) {
  ...
  if (function == PMAC_C_PollAllNow_) {
    ...
    status = (this->immediateWriteRead("&*a", response) == asynSuccess) && status;
    ...
    // AS_211126 - NO NEED TO removed this behaviour since pAxis->stop(0) FUNCTION IS MODIFIED NOT TO re-enables already killed axes in the same CS
    pAxis->stop(0);
    ...
    this->setDebugLevel(level, axisNo, csNo); //this is currently a hack to get debug working for powerpmac, TODO: fix for geo
    pHardware_->setDebugLevel(level, axisNo, csNo); // also debug hardware.
...
asynStatus pmacController::makeCSDemandsConsistent() {
  ...
  /* ALWAYS reset all demands [MSP-237] */
  this->csResetAllDemands = true;
    ...
    if (pCSControllers_[csNum] != NULL) {
      ...
      for (rawAxisIndex = 1; rawAxisIndex <= numAxes_; rawAxisIndex++) {
        ...
        if (aPtr != NULL) {
          ...
          // any real axis can be used in only one CS AT A TIME
          if (csNum == aPtr->getAxisCSNo()) {
            ...
            if (strcmp(axisAssignment, "") != 0) {
              // !!?? there are multiple csAxisAssignmentNo per axisAssignment
              // this would have never worked!  unsigned long uCsAxisNo = axesString.find(axisAssignment);
              int uCsAxisNo = axesString.length();
              size_t c = strlen(axisAssignment);
              while (c > 0){
                c -= 1;
                uCsAxisNo = axesString.find(axisAssignment[c]);
                if (uCsAxisNo > 0) {
                  ...
                  if (aPtr->csRawMoveInitiated_ || this->csResetAllDemands) {
                    ...
                    if (this->csResetAllDemands) {
                      sprintf(command, "&%dQ%d=Q%d", csNum, qvar, pos_qvar);
                    } else {
