...
asynStatus pmacMessageBroker::lowLevelWriteRead(const char *command, char *response) {
  ...
  if (status != asynSuccess) {
    ...
    if (powerPMAC_) {
      ...
      /* AS2 - [MSP-226]
        this routine cleans up the response from extra \r characters
        if this routine is bypassed, the random \r errors cause frequent response dumps at pmacCommandStore level
      */
      if (cleanUp(response) > 0){
        debug(DEBUG_ERROR, functionName, "*** Warning *** extra \\r characters removed from pmac response");
      }
      /* --------*/
...
int pmacMessageBroker::count(const char *str, char ch1) {
  int counts = 0;
  while (*str != '\0') {
    if (*str == ch1) {
      counts++;
...
int pmacMessageBroker::cleanUp(char *str) {
  int counts = 0;
  char* trmstr = str;
  int i=0;
  char lastchar = '\0';
  int extras_removed = 0;
  ...
  while (*str != '\0') {
    ...
    if (*str != '\n') {
      if (*str == '\r'){
        if ((lastchar == '\r') || (lastchar == '\0')){
          // this is not a place for a \r, clean it up
          extras_removed++;
        } else {
          trmstr[i] = *str;
        }
