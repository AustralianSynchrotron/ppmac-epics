...
asynStatus pmacAxis::move(double position, int relative, double min_velocity, double max_velocity,
  ...
  if (pC_->movesDeferred_ == 0) {
    sprintf(command, "%s%s#%d %s%.15f", vel_buff, acc_buff, axisNo_,
...
asynStatus pmacAxis::getAxisStatus(pmacCommandStore *sPtr) {
    ...
    if (pC_->initialised_ && pC_->connected_) {
        ...
        if (retStatus == asynSuccess) {
            ...
            if (encoder_axis_ == 0) {
                setDoubleParam(pC_->PMAC_C_MotorFError_, position);
            ...
            /* AS [MSP-254] Use pmac2 mxx39 (AmpFault) variable to indicate a Hardware Problem 
            */
            sprintf(key, "m%d39", axisNo_);
            if ( not sPtr->checkForItem(key) ) {
                sPtr->addItem(key);
                value = "0";
            }
            else{
            ...
            // reusing
            sscanf(value.c_str(), "%d", &limitsDisabledBit);
            if (limitsDisabledBit > 0) {
                axisProblemFlag = 1;
            }
            /* -----*/
